	...
	if (elements.downloadBtn) {
		elements.downloadBtn.addEventListener('click', () => {
			if (elements.downloadBtn.classList.contains('btn-locked')) {
				alert('Please run all cells first to download the experiment.');
				return;
			}
			downloadExperiment();
		});
...
async function executeCell(cellNumber) {
	...
	// Enable the next cell's Run button
	if (cellNumber < CONFIG.TOTAL_STEPS) {
		const nextCell = elements.cells[cellNumber];
		if (nextCell) {
			const nextRunBtn = nextCell.querySelector('.run-btn');
			if (nextRunBtn) {
...
function resetExperiment(preserveSettings = false) {
	...
	elements.cells.forEach((cell, index) => {
		...
		const runBtn = cell.querySelector('.run-btn');
		if (runBtn) {
			runBtn.disabled = (index > 0); // Only Cell 1 enabled
